...
export const MeetingProvider: React.FC<{ children: React.ReactNode }> = ({
    ...
    try {
      ...
      socketRef.current.on("user-joined", async ({ userId, userName }) => {
        if (participants.length >= 6) {
          socketRef.current?.emit("meeting-full", { to: userId });
          return;
        }
      ...
      socketRef.current.on("meeting-full", () => {
        toast.error("Meeting is full (max 6 participants)");
        cleanupMeeting();
        navigate("/user/meetinghome");
      });
  ...
  const sendMessage = (text: string) => {
    ...
    setMessages((prev) => [...prev, newMessage]);
    socketRef.current?.emit("message", { meetingId, message: newMessage });
  };
  ...
  const toggleScreenShare = async () => {
    try {
      ...
      setParticipants((prev) =>
        prev.map((p) => {
          if (p.id === user?._id) {
            // Create an updated participant with the same interface structure
            const updatedParticipant: Participant = {
              ...p,
              stream: localStream || undefined, // Convert null to undefined to match interface
            };
