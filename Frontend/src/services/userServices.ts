...
//   if (!s3KeyOrUrl) {
//     throw new Error("S3 key or URL is required");
//   }
...
//       console.warn("Falling back to original S3 URL");
//       return s3KeyOrUrl;
//     }
...
//     console.log("Making request to /media/signed-url with s3Key:", s3KeyOrUrl);
...
// // Helper function to get multiple signed URLs at once
// export const getBatchSignedUrls = async (s3KeysOrUrls: string[]): Promise<Record<string, string>> => {
//   if (!s3KeysOrUrls || s3KeysOrUrls.length === 0) {
//     return {};
//   }

//   // Filter out non-S3 URLs and already cached URLs
//   const s3Urls = s3KeysOrUrls.filter(url =>
//     url && (url.includes('amazonaws.com') || (!url.startsWith('http')))
//   );
...
//   }
// };
// Enhanced getSignedUrl function in userServices.ts
export const getSignedUrl = async (s3KeyOrUrl: string): Promise<string> => {
