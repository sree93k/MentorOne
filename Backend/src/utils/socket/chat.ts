...
export const initializeChatSocket = async (chatNamespace: Server) => {
  ...
  // ✅ FIXED: Chat socket middleware - Read token from cookies
  chatNamespace.use((socket: CustomSocket, next: (err?: Error) => void) => {
    // ✅ FIXED: Read token from cookies instead of auth object
    console.log("🔍 Chat socket middleware: Checking authentication");
    ...
    if (!cookieHeader) {
      console.error("❌ Chat auth error: No cookies provided");
    ...
    if (!token) {
      console.error("❌ Chat auth error: No accessToken in cookies");
    ...
    try {
      ...
      socket.data.user = decoded;
      console.log(
        "✅ Chat socket authentication successful for user:",
        decoded.id
      );
      next();
    } catch (error: any) {
