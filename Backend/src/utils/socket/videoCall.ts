...
export const initializeVideoSocket = async (httpServer: any) => {
  ...
  io.on("connection", async (socket: CustomSocket) => {
    ...
    console.log(`Video call user connected: ${userId} as ${role}`);
    ...
    //     console.log(`User ${joiningUserId} joined meeting room: ${meetingId}`);

    //     // Add user to meeting participants
    //     await videoCallService.joinMeeting(meetingId, joiningUserId);
    //     console.log(`User ${joiningUserId} added to meeting ${meetingId}`);

    //     // Store peerId in Redis for signaling
    //     await pubClient.set(
    ...
    //     });
    //     console.log(
    //       `Emitted user-joined for ${joiningUserId} to meeting_${meetingId}`
    //     );
    ...
    socket.on("join-meeting", async (payload, callback) => {
      ...
      try {
        socket.join(`meeting_${meetingId}`);
        console.log(`User ${joiningUserId} joined room: meeting_${meetingId}`);

        // Log all sockets in the room for debugging
        const socketsInRoom = await io.in(`meeting_${meetingId}`).allSockets();
        console.log(`Sockets in meeting_${meetingId}:`, socketsInRoom);
        ...
        console.log("joined userud", joiningUserId);
