...
//     try {
//       const { page, limit, role, status, searchQuery } =
//         QuerySchema.parse(query);
//       logger.info("Fetching all users", { query });

//       const dbQuery: any = {};
//       if (role && role !== "all") {
//         dbQuery.role = { $in: [role] };
//       }
//       if (status) {
//         dbQuery.isBlocked = status === "blocked";
//       }
//       if (searchQuery) {
//         dbQuery.$or = [
...
//         this.userRepository.findMany(dbQuery, page, limit),
//         this.userRepository.countDocuments(dbQuery),
//       ]);
