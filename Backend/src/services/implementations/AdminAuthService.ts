...
// //       );
// //     }
// //     logger.warn("login service admin step 1");
// //     const adminFound = await this.adminRepository.findByEmail(dto.adminEmail);
// //     logger.warn("login service admin step 2");
// //     if (
// //       !adminFound ||
...
// //       );
// //     }
// //     logger.warn("login service admin step 3");
// //     const id = adminFound._id.toString();
// //     const role = Array.isArray(adminFound.role)
// //       ? adminFound.role[0]
// //       : adminFound.role || "admin";
// //     logger.warn("login service admin step 4");
// //     const accessToken = generateAccessToken({ id, role });
// //     logger.warn("login service admin step 5");
// //     const refreshToken = generateRefreshToken({ id, role });
// //     logger.warn("login service admin step 6");
...
//       );
//     }
//     logger.debug("login service admin step 1");
//     const adminFound = await this.adminRepository.findByEmail(dto.adminEmail);
//     logger.debug("login service admin step 2");
//     if (
//       !adminFound ||
//       !adminFound.adminPassword ||
//       !(await bcrypt.compare(dto.adminPassword, adminFound.adminPassword))
...
//       );
//     }
//     logger.debug("login service admin step 3");
//     const id = adminFound._id.toString();
//     const role = Array.isArray(adminFound.role)
//       ? adminFound.role[0]
//       : adminFound.role || "admin";
//     logger.debug("login service admin step 4");

//     try {
//       const accessToken = generateAccessToken({ id, role });
//       logger.debug("login service admin step 5");
//       const refreshToken = generateRefreshToken({ id, role });
//       logger.debug("login service admin step 6");
//       await this.redisTokenService.saveRefreshToken(id, refreshToken, 7);
//       logger.info("Tokens generated and stored", { userId: id });
...
export class AdminAuthService implements IAdminAuthService {
  ...
  async login(dto: AdminLoginDTO): Promise<{
    ...
    logger.info("Admin login service step 1");
    const adminFound = await this.adminRepository.findByEmail(dto.adminEmail);
    logger.info("Admin login service step 2");
    if (
      !adminFound ||
      !adminFound.adminPassword ||
      !(await bcrypt.compare(dto.adminPassword, adminFound.adminPassword))
    ...
    logger.info("Admin login service step 3");
    const id = adminFound._id.toString();
    const role = Array.isArray(adminFound.role)
      ? adminFound.role[0]
      : adminFound.role || "admin";
    ...
    try {
      logger.info("Admin login service step 4");
      const accessToken = generateAccessToken({ id, role });
      logger.info("Admin login service step 5");
      const refreshToken = generateRefreshToken({ id, role });
      logger.info("Admin login service step 6");
      await this.redisTokenService.saveRefreshToken(id, refreshToken, 7);
      logger.info("Tokens generated and stored", { userId: id });

      const adminObject = adminFound.toObject();
      logger.info("Admin login service step 7");
      const { adminPassword, ...adminWithoutPassword } = adminObject;
      logger.info("Admin login service step 8");
      return { accessToken, refreshToken, adminFound: adminWithoutPassword };
    } catch (error) {
      logger.error("Error generating tokens", {
        email: dto.adminEmail,
