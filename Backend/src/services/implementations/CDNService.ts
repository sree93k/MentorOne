...
class CDNService {
  ...
  public getHLSPlaylistURL(
    ...
    const path = `/cdn/hls-playlist/${serviceId}${
      episodeId ? `/${episodeId}` : ""
    }`;
  ...
  public getVideoInfoURL(serviceId: string, episodeId?: string): string {
    ...
    const path = `/cdn/video-info/${serviceId}${
      episodeId ? `/${episodeId}` : ""
    }`;
   ...
   */
  private getOriginPlaylistURL(
    serviceId: string,
    episodeId?: string,
    sessionToken?: string
  ): string {
    const baseUrl = process.env.BACKEND_URL || "http://localhost:5002";
    const path = `/seeker/hls-playlist/${serviceId}${
      episodeId ? `/${episodeId}` : ""
    }`;
    const params = sessionToken ? `?sessionToken=${sessionToken}` : "";
  ...
  private getOriginVideoInfoURL(serviceId: string, episodeId?: string): string {
    const baseUrl = process.env.BACKEND_URL || "http://localhost:5002";
    const path = `/seeker/video-streaming-info/${serviceId}${
      episodeId ? `/${episodeId}` : ""
    }`;
