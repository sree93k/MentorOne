...
app.use(
  cors({
    origin: [
      ...
      "http://localhost:5173",
      "http://localhost:5174" // Add port 5174 for video requests
    ],
    credentials: true,
    methods: ["GET", "POST", "PATCH", "PUT", "DELETE", "OPTIONS"],
    allowedHeaders: ["Content-Type", "Authorization", "Stripe-Signature", "X-Session-Token", "Range"],
  })
);
...
// ✅ ADD: Middleware to attach Socket.IO instance to requests
...
const startServer = async (): Promise<void> => {
  try {
    ...
    // ✅ ADD: Initialize contact socket extension
    console.log("🌐 Extending notification socket for contact messages...");
    const { extendNotificationSocketForContact } = await import(
      "./utils/socket/contactSocket"
    );
    extendNotificationSocketForContact(notificationNamespace);
    console.log("✅ Contact socket extension initialized");
